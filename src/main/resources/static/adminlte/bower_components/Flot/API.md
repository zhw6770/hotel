...
```js
yaxis: {
    transformorderction (v) { return -v; },
...
hooks in the plugins bundled with Flot.
 ...
 - bindEvents  [phase 6]
    ...
    order wrong. The hook only gets one order, though (either for the
...
(make sure you get the order right), then optionally run itorderugh a
